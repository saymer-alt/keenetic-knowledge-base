Да, если в настройках камеры нет выбора кодека, значит, G.711 (PCM MU-LAW) зашит в прошивку намертво для максимальной совместимости. С этим ничего не поделать, но главное, что звук пишется, а видео вообще в идеальном качестве. Так что **да, всё работает на 100% штатно и стабильно!**

Идея выложить это на GitHub — отличная. Это готовый, легковесный и очень надежный проект NVR (сетевого видеорегистратора), который пригодится многим владельцам роутеров. Тем более, на твоем Keenetic Ultra Pro с процессором Filogic эта связка показала нулевую нагрузку, что будет полезно узнать другим инженерам.

Я подготовил для твоего профиля `saymer-alt` готовый шаблон `README.md` и очищенные от личных данных (паролей, IP и точных ID дисков) скрипты.

Можешь прямо копировать и вставлять в свой репозиторий.
