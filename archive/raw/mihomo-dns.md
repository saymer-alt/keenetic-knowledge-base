> **⚠️ Research-снимок, не каноническая инструкция (2026-09-22).**
> Этот файл — сохранённый исследовательский диалог: предложенная архитектура,
> незавершённые эксперименты и исторические рассуждения. Часть утверждений проверена
> позже по официальным источникам, часть — нет.
> Поддерживаемые выводы извлечены в
> [`docs/keenetic-dns-via-mihomo.md`](../../docs/keenetic-dns-via-mihomo.md)
> (с разведением Confirmed / Observed / Research, проверка 2026-09-24).
> Сырые команды из этого файла не являются production-процедурой — не выполнять их
> на живом роутере без отдельной задачи оператора и проверки по текущему справочнику.
